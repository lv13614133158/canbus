...
int main(int argc, char** argv)
{
    ...
    //     "sta", 10, send_132_SteerAngleTarget_callback);
   ...
   velocity_Status_pub=node->create_publisher<autoware_vehicle_msgs::msg::VelocityReport>("/vehicle/status/velocity_status", rclcpp::QoS{1});//速度状态
    ...
    int m_run1=1;
    int m_run2=1;
   	pthread_t recv_threadid,send_threadid,topic_threadid;
    canbus_write_msg_init();
	pthread_create(&recv_threadid,NULL,receive_func,&m_run0);	
    pthread_create(&send_threadid,NULL,send_func,&m_run1);
    pthread_create(&topic_threadid,NULL,topic_func,&m_run2);
